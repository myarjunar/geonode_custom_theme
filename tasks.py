...
http://{public_fqdn}/ >> {override_fn}".format(**envs), pty=True)
    ...
    try:
        current_allowed = ast.literal_eval(os.getenv('ALLOWED_HOSTS') or \
                                           "['{public_fqdn}', '{public_host}', 'localhost', 'django', 'geonode_custom_theme',]".format(**envs))
    except ValueError:
        current_allowed = []
...
def _localsettings():
    settings = os.getenv('DJANGO_SETTINGS_MODULE', 'geonode_custom_theme.settings')
    return settings
...
def _geonode_public_port():
    ...
    if not gn_pub_port:
        gn_pub_port = _container_exposed_port(
            'nginx',
            os.getenv('GEONODE_INSTANCE_NAME', 'geonode_custom_theme')
        )
    return gn_pub_port
