...
RUN mkdir -p /usr/src/geonode_custom_theme

WORKDIR /usr/src/geonode_custom_theme
...
COPY . /usr/src/geonode_custom_theme

RUN chmod +x /usr/src/geonode_custom_theme/tasks.py \
    && chmod +x /usr/src/geonode_custom_theme/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/geonode_custom_theme/entrypoint.sh"]
